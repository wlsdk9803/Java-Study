...
해당 메서드는 멤버 변수도 없고, 단순히 기능만 제공할 뿐이다.  
인스턴스가 필요한 이유는 멤버 변수(인스턴스 변수)등을 사용하는 목적이 큰데, 이 경우 굳이 인스턴스를 위한 메모리를 할당해야 할까? => no

`static` 키워드를 사용하여 객체를 메모리에 올리지 않고도 해당 메서드를 사용할 수 있다..


#### 클래스 메서드 
메서드 앞에도 `static` 을 붙일 수 있다.   
이것을 **정적 메서드** 또는 **클래스 메서드**라 한다.   
정적 메서드라는 용어는 `static` 이 정적이라는 뜻이기 때문이고, 클래스 메서드라는 용어는 인스턴스 생성 없이 마치 클래스에 있는 메서드를
...
- `static` 메서드는 `static` 만 사용할 수 있다.
  ...
  - 인스턴스처럼 참조값의 개념이 없기 때문이다.
  - 객체의 참조값을 직접 매개변수로 전달 시 인스턴스 변수나 메서드를 호출 가능
- 반대로 모든 곳에서 `static` 을 호출할 수 있다.
  - 정적 메서드는 공용 기능이다.
  - 접근 제어자만 허락한다면 클래스를 통해 모든 곳에서 `static` 을 호 출할 수 있다.
...
> 정적 변수는 객체 생성이 필요 없이 메서드의 호출만으로 필요한 기능을 수행할 때 주로 사용  
